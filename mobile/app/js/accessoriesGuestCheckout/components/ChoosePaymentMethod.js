...
class ChoosePaymentMethod extends Component {
  ...
  componentWillMount() {
    ...
    if (paymentOptions.length === 0) {
      hashHistory.push(this.props.giftCardFlow ? '/addNewCard/giftCard' : '/addNewCard');
    }
  ...
  onChoosePaymentMethod() {
    ...
    if (selectedPaymentInfo.paymentType === 'digitalwallet') {
      ...
      if (giftCardFlow) {
        hashHistory.push('/addNewCard/giftCard');
      } else {
        hashHistory.push('/addNewCard');
      }
  ...
  renderOptions() {
    ...
    const renderedOptions = paymentOptions.map((paymentOpt, id) => {
      ...
      if (paymentOpt.paymentType === 'BTA') {
        ...
        // Bill to account
        title = <p className="margin3 onlyTopMargin">{cqContent.label.OD_CHECKOUT_ADD_NEW_CREDIT_CARD_OVERLAY_MSG}</p>;
        value = 'AddCard';
      } else {
        return null;
      }
      ...
      return (
        <div
          className={`pad36 noSidePad verticalAlignMiddle floatLeft clearfix width100 ${id + 1 !== paymentOptions.length ? 'border_EB onlyBottomBorder' : ''}`}
          key={id}
        >
          <RadioButton
            name="contractradio"
  ...
  render() {
    const {
      cqContent, isFetching, dueToday, paymentOptions, checkoutStates, testVersion, showLoaderMessage,
    } = this.props;
    ...
    return (
      <div className="pad12 onlyTopPad margin66 onlyBottomMargin">
        <DigitalWalletModal
          showModal={this.state.showModal}
          closeFn={this.closeDigitalWalletModal}
        />
        {isFetching === true && <Loader content={_lodermessage} />}
        {isValidPayment && !this.props.pastDuePaymentRequired && !checkoutStates.paymentRequired && this.props.stepsCompleted.paymentInfo &&
          <BackButton to="/" />
        }
        <section className="pad20 noBottomPad">
          <Title className="noSidePad">{cqContent.label.OD_CHECKOUT_PAYMENT_METHOD_TITLE_TEXT}</Title>
        ...
        <footer className="noSideMargin pad24 onlySidePad footerFixed">
