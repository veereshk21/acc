      ...
      if (days > 0) { // helps detect if there 0 days left. If so we shouldnt show days anymore, just hour, min, sec
        format = (
          <span className={`black fontTextMedium fontSize_1 color_000 ${cssClass}`} style={styleName}>
            <span style={{ width: '45px' }}>{days}<sup className="fontSize_2">D </sup></span>
            <span style={{ width: '45px' }}>{hours}<sup className="fontSize_2">H </sup></span>
            <span style={{ width: '45px' }}>{mins}<sup className="fontSize_2">M</sup></span>
          </span>);
        // {days + ':' + hours + ':' + mins}
      } else {
        format = (
          <span className={`black fontTextMedium fontSize_1 color_000 ${cssClass}`} style={styleName}>
            <span className={`black fontTextMedium fontSize_1 color_000 ${cssClass}`} style={styleName}>
              <span style={{ width: '40px', display: 'inline-block' }}>{hours}:</span>
              <span style={{ width: '40px', display: 'inline-block' }}>{mins}:</span>
              <span style={{ width: '40px', display: 'inline-block', textAlign: 'left' }}>{secs}</span>
            </span>
          </span>);
      }
      return format;
