...
export const protectionChangeList = (state = {}, action) => {
  switch (action.type) {
    case RESET_PROTECTION_STATE: {
...
export const editState = (state = {}, action) => {
  switch (action.type) {
    ...
    case RESET_STATE: {
      const { checkoutStates } = action.data;
      const newState = {
        ...state.toJS(),
        [EDIT_STATE.SHIPPING]: !!(checkoutStates && (checkoutStates.contactInfoRequired)),
        [EDIT_STATE.PAYMENT]: !!(checkoutStates && checkoutStates.paymentRequired),
        [EDIT_STATE.DELIVERY]: !!(checkoutStates && checkoutStates.shippingMethodRequired),
      };
      return fromJS(newState);
    }
    default:
...
export const protectionModalStatus = (state = false, action) => {
  switch (action.type) {
    case SHOW_PROTECTION_MODAL: {
