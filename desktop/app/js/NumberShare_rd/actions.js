...
export const changeEmergencyContactInfo = (output) => (
  {
    type: SET_E911_UPDATE_BILL_DATA,
    output,
  }
);


export function emergencyInfoLLP() {
  return function emergencyAddressRedirect() { hashHistory.push('/e911'); };
}

export function indexLLP() {
  return function emergencyAddressRedirect() { hashHistory.push('/'); };
}
...
export function addDeviceShare(productInfo) {
  return function submitNumberShare(dispatch) {
    ...
    addDeviceShareAPI(productInfo).then((response) => {
      dispatch(asyncFetchSucess());// action to hide loader
      if (response.data.statusCode === API_SUCCESS_CODE) {
        if (response.data.errorMap && Object.keys(response.data.errorMap).length > 0) {
          dispatch(showErrorNotification(getErrorMap(response.data.errorMap)));
          // dispatch(setEmergencyContactInfo(response.data.output));
          // dispatch(toggleEmergencyContactModal(true));
        } else if (response.data.output && response.data.output.alternativeAddressExist) {
          dispatch(changeEmergencyContactInfo(response.data.output));
          dispatch(toggleEmergencyContactModal(true));
        } else {
